...
- **КНОПКИ — развилки давай выбором, не текстом.** В Code/Cowork нетехнический юзер не должен
  печатать команды. Где есть **настоящий выбор** — выводи его как кликабельные опции (механизм
  `AskUserQuestion`, рендерится кнопками), а не списком, который надо перепечатать. Применять:
  - **меню «что умеешь»** — топ-рецепты кнопками (`recipes.json` поле `short` = подпись на кнопке);
  - **шаг онбординга** (`next_step`) — «собрать Принцепс / добавить советника / готов» кнопками;
  - **предложения в конце заседания** — «занести в журнал · покажи разбор · готово» кнопками.
  Лимит ~4 опции на вопрос (клиент сам добавит «Другое» для остального) → клади самые частые,
  подписи короткие (1–3 слова). Кнопки — для РЕШЕНИЙ; где хватает утверждения, не навязывай выбор.
  Скилл не рисует свой UI — это единственный «кнопочный» механизм; плюс слэш-команды `/board *`.
