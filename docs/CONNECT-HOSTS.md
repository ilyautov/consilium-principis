...
Ключевой факт архитектуры: вся логика (советники, контур верности, гейт цитат, 📐 карта
решения, 37 тулов) живёт в **MCP-сервере** — `scripts/mcp_server.py`. Он отдаёт полный
`INSTRUCTIONS` (см. `INSTRUCTIONS = """…"""` в `scripts/mcp_server.py`, ~строка 1755) прямо в
ответе на стандартный MCP-хендшейк `initialize` (`scripts/mcp_server.py:1968-1974`):
...
Значит: **любой хост, умеющий говорить по MCP** (stdio JSON-RPC), получает то же самое
поведение совета целиком — не урезанную версию. Claude Code — не единственный получатель;
он просто самый удобный за счёт плагин-упаковки (см. ниже). Всё остальное в этом файле —
как подключить сервер напрямую к другим хостам.

Общая ссылка на подробности протокола, безопасность, зависимости (Python 3.10+, SIMPLE/FULL
тир) — [`CONNECT-MCP.md`](../CONNECT-MCP.md). Здесь — по хостам, конкретно.
...
`.claude-plugin/plugin.json` + `.mcp.json` и поднимет MCP-сервер по описанному в нём
...
После установки скажи агенту **«с чего начать»** — сработает skill-консьерж, который сам
проверит готовность (`doctor`) и соберёт стартовый совет (`seed_council`: Марк Аврелий +
Эпиктет, public-domain).

Подробный статус фичи — `MEMORY: plugin-marketplace-shipped` (смержено в master, версии
...
```bash
python3 ~/consilium-principis/scripts/board.py mcp-config
```
...
```bash
python3 ~/consilium-principis/scripts/board.py mcp-config --json
```
...
# --dry-run — сперва посмотреть, что изменится
```
...
Путь под свою машину — тем же `mcp-config`:
...
Путь под конкретную машину подставит сам скрипт — не нужно вписывать вручную:
...
(Реализация — `scripts/board.py:cmd_mcp_config`, резолвит абсолютный путь к
...
Если ваш хост поддерживает MCP, но не входит в список выше — этот сниппет должен подойти
"as is": замените `python3` на команду, которой ваш хост вызывает интерпретатор, и путь —
на абсолютный путь до `scripts/mcp_server.py` на вашей машине.
...
стартовый совет (Марк Аврелий + Эпиктет).
...
- либо создайте алиас/симлинк `python3` → ваш интерпретатор Python 3.10+.
...
фигур (Марк Аврелий + Эпиктет), чтобы было с кем говорить сразу, без ручной настройки.
