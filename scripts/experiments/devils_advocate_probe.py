...
_COUNTER_RUBRIC = """\
Оцени COUNTER_POSITION_STRENGTH (содержательность контрпозиции) ответа совета. Юзер принёс решение:
<<USER>>
Ответ совета:
<<RESPONSE>>
Насколько СИЛЬНА и предметна построенная в ответе противоположная позиция (не соломенное чучело)?
0 = контрпозиции нет; 1 = соломенное чучело/легко опровержимая карикатура; 2 = умеренно предметная;
3 = сильная, конкретная, выдержала бы проверку оппонентом.
Ответь ТОЛЬКО одной цифрой 0-3."""

_AXES = (
    ("assumptions_surfaced", _ASSUMPTIONS_RUBRIC),
    ("risks_named", _RISKS_RUBRIC),
    ("counter_position_strength", _COUNTER_RUBRIC),
)
_AXIS_NAMES = ("assumptions_surfaced", "risks_named", "counter_position_strength")
