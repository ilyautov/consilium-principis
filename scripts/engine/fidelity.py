...
# Разделители предложений: скоуп гарда — предложение (см. ниже). ТОЛЬКО терминальная
# пунктуация [.!?…;\n]: «:» и кавычки НЕ разделители — иначе отрицание до двоеточия/
# кавычек отрывалось от цитаты («did not recommend: win by cheating…» давало 🔵,
# финальное ревью 2026-07-20, Important). Цена — ложные 🟡 на репортируемой речи
# («X сказал: …» с «not» у X топит чистую цитату в той же фразе): осознанный
# fail-closed bias, гард лучше перестрахуется, чем перевернёт смысл.
...
def _crop_severs_negation(q: str, raw_text: str, quote: str = None) -> bool:
    ...
    В каждом предложении проверяется префикс ПЕРВОЙ встречи q (str.find); последующие
    вхождения в том же предложении не различаются — если первая встреча под отрицанием,
