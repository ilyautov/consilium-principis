...
Решения (задокументированы, ревью Ф1):
  ...
  • Деление на ноль и переполнение результата обнаруживаются на eval →
    SafeExprEvalError; вызывающий код (mc_run) обязан уронить ВЕСЬ расчёт
    (fail-closed): формула, делящаяся на ноль внутри подтверждённых диапазонов,
    — дефект модели, юзер должен её починить. Переполнение: чисто-целочисленная
    арифметика растит bigint неограниченно (~20 литералов по 19 цифр → ~1e380),
    и float() над ним бросал сырой OverflowError мимо RU-контракта (ревью M5).
...
class SafeExprEvalError(ValueError):
    ...
    переполнение результата за пределами float-диапазона)."""
...
def compile_expr(expr, allowed_names):
    ...
    def evaluator(env):
        try:
            ...
            # bigint-результат крупнее float-диапазона (чисто-целочисленная
            # арифметика: ~20 литералов по 19 цифр → ~1e380): та же eval-ошибка,
            # что и деление на ноль, — RU-текст, не сырой OverflowError (ревью M5)
            raise SafeExprEvalError(
                "Число в формуле переполнило расчёт — упростите формулу или масштаб "
                "величин.")
