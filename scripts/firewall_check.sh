...
# СЛЕПАЯ ЗОНА (осознанно): денайлист строится ДИНАМИЧЕСКИ из advisors/, поэтому имена УДАЛЁННЫХ
# из доски фигур (напр. naval/munger как ПРИМЕРЫ в докстрингах shipped-скриптов) сюда НЕ попадают
# и не сканируются. Принято владельцем (примеры оставлены намеренно) — сознательный компромисс,
# а не дыра. Не поднимай тревогу без нового факта.
