[English](CONNECT-MCP.en.md) · **Русский**
...
## Подключение за 3 шага (Claude Desktop и другие MCP-хосты)

> **Cowork не проходил отдельный smoke-тест.** Не считай эти шаги подтверждением его поддержки:
> сначала проверь подключение локально и сообщи результат через [`SUPPORT.md`](SUPPORT.md).
...
Нужна папка с проектом на **той же машине**, где локальный MCP-хост запускает сервер: так личный
корпус остаётся локально. Для Cowork этот способ запуска и модель разрешений не подтверждены.
...
Проще всего попросить локального агента выполнить эту строку или вставить её в терминал самому:
```bash
...
## Вариант: `.mcpb`-бандл (расширение Claude Desktop)

Кроме скрипта выше, релиз отдаёт **`.mcpb`-бандл** — однофайловое расширение для Claude Desktop
(меню Extensions), если не хочешь править конфиг руками.

**Установить:**
1. Скачай `consilium-principis.mcpb` со [страницы релиза](https://github.com/ilyautov/consilium-principis/releases/latest).
2. Claude Desktop → **Settings → Extensions** → **Install from file…** (или перетащи `.mcpb` в окно Extensions).
3. Подтверди и **полностью перезапусти** Claude Desktop (у stdio-сервера нет горячей перезагрузки).

**Обновить:** скачай `.mcpb` новой версии и установи так же — Desktop заменит прежнюю; затем рестарт.
Текущую версию покажет тул `explain_self`.

**Удалить:** Settings → Extensions → Consilium Principis → **Remove/Uninstall**, затем рестарт Desktop.

> **Кавеат (честно):** бандл НЕ несёт собственный Python — его `manifest.json` запускает системный
> `python3` (на Windows `py -3`). Значит нужен установленный Python 3.10+ (см. «Зависимости» ниже);
> если его нет, расширение поставится, но сервер не стартует. И бандл везёт только машинерию —
> состав советников и корпуса собери локально после установки (`seed_council` для PD-мудрецов).

## Безопасность: граница в коде сервера

В хостах, которые запускают локальный stdio MCP-сервер, процесс получает права файловой системы и
сети твоего пользователя. Для Cowork этот режим запуска и его песочница отдельно не проверялись,
поэтому не делай выводов о границах доступа без собственной проверки. В любом случае **легальную и
безопасную границу держи в самом сервере**: фетч только public-domain, запись только в
`advisors/*/sources/` и `principis_corpus/`, как уже сделано (см. правила в SKILL.md). Доступ
широкий, дисциплину задаёт код.
