    ...
    if ranked:
        ...
        out.update({"quotes": ranked,
                ...
                "note": ("Вставь любой из `quotes` как есть в opinion.quote (marker подтверждён гейтом). "
                         ...
                         "находок больше. Формулируй так, как ЗВУЧАЛА БЫ САМА ЦИТАТА, а не как тему: "
                         "тема ловит того, кто пишет о предмете прямым текстом (комментатора), а "
                         "советник говорит косвенно — попаданий впятеро больше.")})
        return out
    out.update({"quotes": [], "best": None, "marker": "🟡",
            "note": ("Дословного нет — НЕ выдумывай, иди 🟡. Гейт исправен. ПЕРЕД тем как уйти в 🟡, "
                     "попробуй ещё раз: дай query в языке корпуса (English) и сформулируй так, как "
                     "ЗВУЧАЛА БЫ САМА ЦИТАТА, а не как тему («everyone sees what you appear to be», "
                     "не «public image reputation») — тема ловит комментатора, а не советника; "
                     "попаданий впятеро больше. Не помогло — 🟡 честен.")})
    return out
...
TOOLS = {
    ...
    "cite": {
        "description": "ГОТОВЫЕ 🔵-цитаты под довод (вместо ручной сборки — так цитата не станет "
                       ...
                       "формулировок, давай их в ЯЗЫКЕ КОРПУСА (English) и формулируй так, как "
                       "ЗВУЧАЛА БЫ САМА ЦИТАТА, а не как тему (тема ловит комментатора, советник "
                       "говорит косвенно — попаданий впятеро больше); ретрив "
...
5. КОНТУР ВЕРНОСТИ (протокол-гейт, НЕ нарушай). 🔵 ставь ТОЛЬКО если гейт подтвердил; иначе 🟡.
   ...
   МОЖНО списком из 2-4 формулировок-перефразировок — перевод запроса даёт реальный лифт.
   ФОРМУЛИРУЙ ТАК, КАК ЗВУЧАЛА БЫ САМА ЦИТАТА, а не как ТЕМУ: «everyone sees what you appear to
   be» достаёт первоисточник, «public image reputation» — комментарий О НЁМ. Замер: попаданий в
   нужный пассаж ВПЯТЕРО больше (0.33 против 0.07). Механизм, а не магия: тематический запрос
   ловит того, кто пишет о предмете ПРЯМЫМ ТЕКСТОМ, — то есть комментатора; советник же говорит
   косвенно и метафорично, его достаёт запрос, похожий на его собственную речь. Если
