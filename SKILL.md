...
- **РЕНДЕР ЗАСЕДАНИЯ — ОБЯЗАТЕЛЬНО ВИДЖЕТОМ, НЕ ПРОЗОЙ (хард-правило).** Если в хосте ЕСТЬ
  `mcp__visualize__show_widget` (Cowork) — финальный вердикт совета РЕНДЕРИТСЯ им, а не вываливается
  текстом. Конвейер: собрал canon-объект → `render_session(session, surface=widget, depth=plain)` →
  скорми `content` в `show_widget`. НЕ дублируй заседание прозой рядом с виджетом (премортем,
  легенды маркеров, «ожидаемый исход +N», abstention-бухгалтерия в тексте = тот самый визуальный
  шум — это `depth: expert`, по запросу, и всё равно внутри виджета/свёрнуто). В чате оставь 1–2
  строки-подводку + сам виджет. Текст/markdown (`surface=md`) — ТОЛЬКО для хостов без `show_widget`
  (голый Claude Code). Проверка перед ответом: «совет есть, show_widget есть → я вызвал виджет?»
  Если отрисовал текстом при доступном виджете — это регресс, переделай.
