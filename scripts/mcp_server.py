...
4. КОНТУР ВЕРНОСТИ (протокол-гейт, НЕ нарушай). 🔵 (дословно) ставь ТОЛЬКО если `fidelity_check`
   подтвердил. Иначе 🟡. НИКОГДА не выдумывай цитаты и не приписывай 🔵 без подтверждения.
   ЧТОБЫ БЫЛИ 🔵: бери в quote.text ТОЧНЫЙ текст пассажа из `retrieve` (буква в букву), перевод —
   в quote.translation; гейт подтвердит 🔵. Пересказ/перевод вместо текста → 🟡. Если гейт вернул
   🟡 на том, что считал дословным — текст НЕ точный (перевёл/сократил), это НЕ «дефект корпуса»:
   гейт исправен, возьми ровно строку `text` из retrieve. Не выдумывай дефекты гейта.
