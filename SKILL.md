...
3. **Добавить советника** (где живёт ров — внимательно):
   ...
   б. **Тир-манифест** `sources/manifest.json` — РЕШЕНИЕ о тире принимаешь ты разговором («это
      слова автора → P1; это комментарий Тарасова → S1; предисловие → B»), затем `scaffold_manifest`.
...
- **КНОПКИ — развилки давай кликом, не печатью.** В Code/Cowork нетехнический юзер не должен
  набирать команды. Где есть **настоящий выбор** — давай кликабельные опции. Применять к: меню
  «что умеешь» (`recipes.json`, поле `short` = подпись), шагу онбординга (`next_step`), предложениям
  в конце заседания («журнал · разбор · готово»). Два способа рендера, выбирай по клиенту:
  - **`AskUserQuestion`** (везде) — рендерится чипами-кнопками, выбор возвращается в диалог. НО
    **лимит ~4 опции** (клиент добавит «Другое»). Поэтому **сперва выпиши ВСЕ варианты текстом**
    (полное меню — ничего не прячем за «Другое»), и лишь потом кнопки = быстрый клик по топ-4.
    Кнопки — поверх выписанного меню, не вместо него.
  - **Визуализация** (где клиент умеет, напр. Cowork) — меню можно отрисовать богаче: HTML-карточки/
    канвас вместо чипов. Это лучший ДИСПЛЕЙ (видно всё, не 4); выбор всё равно замыкается через
    `AskUserQuestion` или фразу/слэш-команду. Дисплей и механизм выбора — разные слои.
  Подписи короткие (1–3 слова). Кнопки — для РЕШЕНИЙ; где хватает утверждения, не навязывай выбор.
  Скилл своего UI не рисует сам — это механизмы хоста; плюс слэш-команды `/board *`.
