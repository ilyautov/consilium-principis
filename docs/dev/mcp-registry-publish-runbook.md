...
> **TODO (release-time, ОБЯЗАТЕЛЬНО до `mcp-publisher publish`):** `identifier` в
> `server.json` — это *будущий* URL релиза
> (`…/releases/download/v0.1.0/consilium-principis.mcpb`). Он **404-ит, пока не
> сделан GitHub Release (шаг 3) и репо не флипнут в public** (предусловия внизу).
> Это НЕ фейковый плейсхолдер — это точный финальный URL, но резолвится он только
> после релиза. Перед publish: (1) вписать реальный `fileSha256` вместо сентинела
> (шаг 2), (2) проверить `curl -I <identifier>` → `200`, а не `404`. Не публиковать,
> пока URL не резолвится.
