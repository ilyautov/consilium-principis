...
class Passage:
    ...
    tier: str = None
    # M4: сырой семантический косинус, когда `score` — НЕ косинус (hybrid_alpha-смесь в
    # SemanticEngine или RRF-слияние в HybridEngine). None → score и есть косинус (чистая
    # семантика) либо косинуса нет вовсе (lexical). Гейт релевантности калиброван под
    # косинус и обязан резать полосой raw_score, а не смесь (иначе sub-band камуфляж с
    # высоким токен-оверлапом улетает за band_hi в auto-keep без судьи).
