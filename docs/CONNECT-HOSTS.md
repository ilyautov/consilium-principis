...
решения, 60 тулов) живёт в **MCP-сервере**, то есть в `scripts/mcp_server.py`. Он отдаёт полный
...
поведение совета целиком, а не урезанную версию. Claude Code не единственный получатель;
он просто самый удобный за счёт плагин-упаковки (см. ниже). Всё остальное в этом файле про то,
