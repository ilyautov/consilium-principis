...
**verbatim-нормализация (🔵-сверка)** — 🔵/🟢 требует дословного совпадения по
НОРМАЛИЗАЦИИ (регистро- и пунктуационно-независимо), при этом: ≥3 слов
(`fidelity.MIN_QUOTE_WORDS = 3` — одиночное слово → 🟡, защита от тривиального
совпадения) И обрезка НЕ должна рвать отрицание (`_crop_severs_negation`:
«only to do but» из «not only… but» → 🟡, а не 🔵). То есть «слово-в-слово по
нормализации», не «посимвольно». (`MIN_QUOTE_CHARS = 8` сохранён отдельно — только
для `citation_rate_eval` как мин. длина извлекаемого спана, не гейт верности.)
...
- `auto` (дефолт, 2026-07-24) — **host**: судит сам хост двухфазным протоколом. Работает на
  ЛЮБОЙ машине (без ollama/чат-модели); при сильном хосте (Claude) — точнее и на порядок быстрее
  локального ollama-судьи (тот на gemma3:27b рвал 60с MCP-таймаут при веере совета). Вербатим-🔵
  всё равно детерминирован в коде → host двигает лишь флаг релевантности, не ядро рва. Независимый
  судья (`ollama`/`api`) — ЯВНЫЙ opt-in, «больше приколюх при наличии железа»; auto НИКОГДА не
  уходит в облако (и даже в локальный ollama) без явного выбора.
Явный выбор: fail-closed деградация api→ollama→host. Честный лейбл в `doctor`.
