...
> ⚠️ **ИСТОРИЧЕСКИЙ ДОКУМЕНТ (2026-06-10, начальная передача).** Оставлен как летопись
> первого хендоффа; НЕ отражает текущую реальность (master @ f36e226). С тех пор внедрено:
> двухфазный судья-гейт цитат (`cite`/`gate_verdict`, Moat v2 — 0 misapply), decision-calc трек
> (📐 карта решения + Монте-Карло), петля исхода (predicted-vs-actual), grounded-линза «Стратег»
> (Сунь-цзы 🔵), весь цикл как 37 MCP-тулов, ритуал `make moat-check`. Актуальное состояние —
> `SKILL.md` + INSTRUCTIONS (правила 0-18) в `scripts/mcp_server.py`; роадмап рва —
> `docs/dev/2026-07-02-moat-v2-roadmap.md`; дизайн 📐 — `docs/dev/2026-07-03-decision-map-mc-design.md`.
> Имена советников ниже (munger/naval/marcus-aurelius) — прототипные из первой сборки; шипится
> только public-domain, приватная доска `advisors/` не коммитится (файрвол копирайта/приватности).
