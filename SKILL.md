...
7. **Протокол-гейт (когда ризонишь ТЫ, хост).** Consilium можно поднять как MCP-сервер
   ...
   **ВЕСЬ цикл — через MCP, юзер не выходит из агента.** Не только ризонинг-тулы: сборка и админка
   ...
   операции (build/seed/ingest) синхронны + сеть. Так из Cowork/Code/любого MCP-хоста доступен
   полный жизненный цикл проекта без перехода в терминал. Единственный внешний шаг — однократно
   подключить сам MCP-сервер в хост (chicken-egg: его не подключить им же).
...
- **ВЫБОР и ВИЗУАЛИЗАЦИЯ — дисплей ≠ механизм выбора (разные слои).** Нетехнический юзер не должен
  ...
  плюс слэш-команды `/board *`. В Cowork предпочитай `show_widget`; вне — markdown/HTML-фолбэк.
